...
public enum ExceptionEnum implements IExceptionEnum {
    ...
    _202("202", "结果正在处理中"),
    /**
     * 请求的代理服务器修改了源服务器返回的 200 中的内容，一般用不到。比如说，我们通过代理服务器向服务器 A 请求用户信息，服务器 A
     * 正常响应，但代理服务器命中了缓存并返回了自己的缓存内容，这时候它返回 203 告诉我们这部分信息不一定是最新的，我们可以自行判断并处理。
     */
    _203("203", "返回缓存内容"),
    /**
    ...
    /**
     * 请求成功，但根据请求头只返回了部分内容。比如说，我们下载一部片，共有 10部分，我们把请求也分成了
     * 10次（防止一次请求过大），这时候服务器就可以返回 206并在其头部告诉我们这是哪一部分，然后再根据这个信息进行拼装。
     */
    _206("206", "请求成功，只返回了部分内容"),
    /**
     * 请求成功，但结果有多种选择。比如说，我们下载一部片，服务器有 avi、mp4 等格式，这时候可以返回 300，并在
     * body里告知有哪些格式，然后用户可以根据这些格式再次请求。
     */
    _300("300", "请求成功，根据返回内容选择"),
    /**
     * 请求成功，但是资源被永久转移。比如说，我们要下载葫芦娃，但是由于旧的存储服务商涨价了，现在要使用新的存储服务了，要去新地址下载，这时候可以返回
     * 301，并在 header的 Location中告知新的地址，以后也应当到这个地址下载。
     */
    _301("301", "请求成功，但是资源被永久转移"),
    /**
     * 请求成功，但是资源被临时转移了。和 301不同的是， 除非是 HEAD请求，否则新地址的信息应当在
     * body中返回，并且资源只是临时转移，以后不应当通过新地址来下载。
     */
    ...
    /**
     * 请求的资源并没有被修改过。比如说，我们发送请求想看看 5.20后的情侣信息，服务器查询没有新的情侣信息产生，这时候可以返回
     * 304，然后客户端可以继续用旧的数据。
     */
    _304("304", "请求成功，请求的资源并没有被修改过"),
    /**
     * 请求的资源必须通过代理访问。比如说，我们想请求服务器 A 上新的 iPhone 的信息，但是需要通过代理服务器才能访问， 如果直接请求了服务器
     * A，没有经过代理服务器，这时候服务器 A 就可以返回 305从而告诉我们应当访问代理服务器。
     */
    ...
    _404("404", "请求的内容不存在"),
    /**
     * 请求的方法不允许使用。比如说， 服务器只实现了 PATCH 了局部更新资源，并没有实现 PUT来替换资源， 而我们使用了
     * PUT，这时候服务器可以返回 405 来告知并没有实现对 PUT 的相关处理。
     */
    _405("405", "请求的方法不允许使用"),
    /**
     * 请求的资源并不符合要求。比如说， 我们 header 里请求 JSON 格式的数据， 但是服务器只有 XML 格式的数据，这时候可以返回
     * 406告知。
     */
    ...
    /**
     * 客户端请求超时。我们想 POST\创建一个用户，虽然建立了连接，但是网络不好，服务器在规定时间内没有得到我们的请求信息，这时候服务器可以返回
     * 408告诉我们超时了。然后我们可以重新发送请求。
     */
    _408("408", "客户端请求超时"),
    /**
     * 请求冲突。比如说，服务器要求不同用户不能重名，服务器已经有了一个名叫小伟的用户，这时候我们又想创建一个名叫小伟的用户，服务器可以返回
     * 409，告诉我们冲突了，也可以在 body 中明确告知是什么冲突了。
     */
    _409("409", "请求冲突"),
    /**
    ...
    /**
     * 请求的资源不符合请求头中的 IF-*的某些条件。比如说，我们下载葫芦娃，然后在请求头告知服务器要 5.20后更新过的，服务器没有，于是返回了
     * 412。
     */
    ...
    _501("501", "请求还没有被实现"),
    /**
     * 网关错误。比如说，我们向服务器 A 请求下载葫芦娃，但是 A 其实只是一个代理服务器，他得向 B 请求葫芦娃，但是不知道为啥 B
     * 不理他或者给他错误，这时候哦可以 A 返回 502 用来表示 B 这家伙傲娇了。
     */
    _502("502", "网关错误"),
    /**
    ...
    public static String getNameByIndex(String code) {
        for (ExceptionEnum enumTest : ExceptionEnum.values()) {
            if (enumTest.code == code) {
                return enumTest.message;
            }
        }
        return null;
    }
