...
- [山脉科技共享组件平台](#山脉科技共享组件平台)
    ...
    - [测试接口](#测试接口)
        - [登录](#登录)
    - [对外接口URL请求标准](#对外接口URL请求标准)
    - [关于共享组件发布到应用货架的注意事项:](#关于共享组件发布到应用货架的注意事项:)
    - [关于用户数据和业务数据初始化的注意事项:](#关于用户数据和业务数据初始化的注意事项:)
    - [获取用户登录信息的方式](#获取用户登录信息的方式)
    - [后台接口状态码描述](#后台接口状态码描述)
...
|组件名称|组件工程名|测试服务器组件访问地址|本地运行默认端口|负责人|数据库名称|
|--|--|--|--|--|--|
...
|雨情组件|cbb_pptn|❓|❓|❓|❓|
|天气组件|cbb_weather|192.168.140.155:45004|22225|刘源|cbb_weather|
|测站管理组件|cbb_station|❓|❓|❓|❓|
|消息通知组件|cbb_notification|192.168.140.155:45011|22229|钱云阳|cbb_notification|
|组态通信组件|cbb_scada|❓|❓|刘源|❓|
|数据转移组件|cbb_etl|❓|❓|❓|❓|
|视频组件|cbb_video|❓|❓|❓|❓|
...
|Redis|内存数据库|192.168.140.155|10001|无|无|无|
...
|数据类别|详情|
|:---|:---|
|系统菜单|系统管理，用户管理，角色管理，功能管理，数据字典管理，数据权限管理|
|用户角色|ROLE_SUPERUSER|
|用户账户|admin|

- 在新创建的项目中无需再次录入，可以直接使用，如果项目上有新增加的用户组件初始化数据，<font color=red size=5px>请在项目对应的业务组件中创建用户组件初始化数据插入sql脚本</font>，在之前已有的基础上新增即可，如图：

  ![sql脚本文件](doc/sql_script_file.png)

- 数据初始化到用户组件数据库的sql语句格式如图：

  ![sql脚本文件格式](doc/sql_script_format.png)
